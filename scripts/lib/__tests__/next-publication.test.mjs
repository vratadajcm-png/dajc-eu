...
describe('formatNextPublicationLabel', () => {
  it('is 7 days ahead, in the publication window, with the correct summer timezone abbreviation', () => {
    const label = formatNextPublicationLabel(new Date('2026-08-21T10:25:00Z'));
    expect(label).toBe('Friday, 28 August 2026 between 08:00 and 13:59 CEST');
  });

  it('uses CET in winter', () => {
    const label = formatNextPublicationLabel(new Date('2026-01-16T10:25:00Z'));
    expect(label).toBe('Friday, 23 January 2026 between 08:00 and 13:59 CET');
  });
});
